...
public class I18nUtils {
    ...
    /**
     * Retrieves the i18n key based on the provided string.
     *
     * @param key The string key.
     * @return The i18n key if found, otherwise the original key.
     */
    ...
    /**
     * Fetches a translation based on the provided i18n key and optional arguments.
     *
    ...
    /**
     * Fetches a translation based on the provided string key with the option of fallback to Chinese.
     * This method first looks for the translated key in the Chinese map, and then falls back to
     * the original key if not found.
     *
     * @param key The original key.
     * @param args Arguments for the translation.
     * @return The localized translation, or the formatted original key if not found.
     */
    public static String getByZh(String key, Object... args) {
        String i18nKey = ZH_CN_KEY_MAP.getOrDefault(key, key);  // Use default if not found
        try {
            return I18n.translate(i18nKey, args);
        } catch (Exception e) {
            // Log or handle the exception more gracefully if needed
            return String.format(key, args);  // Return the formatted string if an error occurs
        }
    }
}

...
import com.flechazo.rewards.impl.AdvancementRewardParser;
...
public class AdvancementSelectScreen extends Screen {
    ...
    private final List<AdvancementData> displayableAdvancementList = SakuraSignInFabric.getAdvancementData().stream().filter(o -> o.getAdvancementDisplay().getIcon().getItem() != Items.AIR).toList();
    // 每页显示行数
     ...
     * 当前选择的进度
     */
    private Reward currentAdvancement = new Reward(new Identifier(""), ERewardType.ADVANCEMENT);
    /**
     * 奖励概率
     */
    ...
    public AdvancementSelectScreen(@NonNull Screen callbackScreen, @NonNull Consumer<Reward> onDataReceived, @NonNull Reward defaultAdvancement, Supplier<Boolean> shouldClose) {
        super(Text.literal("AdvancementSelectScreen"));
    ...
    private void updateLayout() {
        ...
        this.OP_BUTTONS.put(OperationButtonType.TYPE.getCode(), new OperationButton (OperationButtonType.TYPE.getCode(), context -> {
            ...
            IText text = this.displayMode ? IText.i18n("列出模式\n有图标的 (%s)", displayableAdvancementList.size()) : IText.i18n("列出模式\n所有进度 (%s)", allAdvancementList.size());
        ...
        this.OP_BUTTONS.put(OperationButtonType.ADVANCEMENT.getCode(), new OperationButton(OperationButtonType.ADVANCEMENT.getCode(), context -> {
            ...
            AbstractGuiUtils.fill(context.graphics(), (int) context.button().getX(), (int) context.button().getY(), (int) context.button().getWidth(), (int) context.button().getHeight(), 0xEE707070, 2);
            AbstractGuiUtils.fillOutLine(context.graphics(), (int) context.button().getX(), (int) context.button().getY(), (int) context.button().getWidth(), (int) context.button().getHeight(), 1, lineColor, 2);
                context.graphics().drawItem(AdvancementRewardParser.getAdvancementData((Identifier) RewardManager.deserializeReward(this.currentAdvancement)).getAdvancementDisplay().getIcon(), (int) context.button().getX() + 2, (int) context.button().getY() + 2);
            context.button().setTooltip(IText.literal(AdvancementRewardParser.getAdvancementData(((Identifier) RewardManager.deserializeReward(this.currentAdvancement))).getAdvancementDisplay().getTitle().getString()));
        ...
        for (int i = 0; i < maxLine; i++) {
            ADVANCEMENT_BUTTONS.add(new OperationButton(i, context -> {
                ...
                if (index >= 0 && index < advancementList.size()) {
                    ...
                    context.graphics().drawItem(advancementData.getAdvancementDisplay().getIcon(), (int) (context.button().getX() + this.margin), (int) context.button().getY());
    ...
    private void handleAdvancement(OperationButton bt, int button, AtomicBoolean flag) {
        if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            if (StringUtils.isNotNullOrEmpty(bt.getId())) {
                ...
                LOGGER.debug("Select effect: {}", AdvancementRewardParser.getAdvancementData(resourceLocation).getAdvancementDisplay().getTitle().getString());
                flag.set(true);
            }
        }
    }
    ...
    private void handleOperation(OperationButton bt, int button, AtomicBoolean flag, AtomicBoolean updateSearchResults) {
        ...
        // 编辑进度Json
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.i18n("请输入进度Json").setShadow(true)
                    , IText.i18n("请输入")
                    , ""

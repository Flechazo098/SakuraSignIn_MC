...
public class ConfirmationScreen extends Screen {
    ...
    public ConfirmationScreen(Screen callbackScreen, IText titleText, @NonNull Runnable onConfirm) {
        super(Text.literal("ConfirmationScreen"));
    ...
    protected void init() {
        ...
        // 创建提交按钮
        ButtonWidget submitButton = AbstractGuiUtils.newButton(this.width / 2 + 5, this.height / 2 + 10, 95, 20, Text.literal(I18nUtils.getByZh("确认")), button -> {
            onConfirm.run();
            MinecraftClient.getInstance().setScreen(previousScreen);
        });
        this.addDrawableChild(submitButton);
        // 创建取消按钮
        this.addDrawableChild(AbstractGuiUtils.newButton(this.width / 2 - 100, this.height / 2 + 10, 95, 20, Text.literal(I18nUtils.getByZh("取消")), button -> {

...
public class SignInCell {
    ...
    /**
     * @see com.flechazo.enums.ESignInStatus
     */
    public int status;
    ...
    public void render(DrawContext graphics, TextRenderer font, int mouseX, int mouseY) {
        ...
        if (showText) {
            ...
            MutableText dayStr = Text.literal(String.valueOf(day));
            if (year == DateUtils.getYearPart(date) && month == DateUtils.getMonthOfDate(date)) {
                if (day == DateUtils.getDayOfMonth(date)) {
                    ...
                    dayStr.setStyle(dayStr.getStyle().withUnderline(true));
                } else {
            ...
            float dayWidth = font.getWidth(dayStr);
            graphics.drawText(font, dayStr, (int) (x + (width - dayWidth) / 2), (int) (y + textureCoordinate.getDateOffset() * this.scale + 0.1f), color, false);
        }
    }
    ...
    public void renderTooltip(DrawContext graphics, TextRenderer font, int mouseX, int mouseY) {
        ...
        MutableText title = Text.literal(String.format("%s %s", monthTitle, dayTitle));
        double fontWidth = font.getWidth(title);
        ...
        graphics.drawText(font, title, (int) tooltipDateX, (int) tooltipDateY, 0xFFFFFF, false);

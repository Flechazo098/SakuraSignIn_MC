...
import com.flechazo.config.ArraySet;
...
import com.flechazo.rewards.impl.ItemRewardParser;
...
public class ItemSelectScreen extends Screen {
    ...
    protected void init() {
        ...
        this.updateLayout();
        // 创建文本输入框
        this.inputField = AbstractGuiUtils.newTextFieldWidget(this.textRenderer, bgX, bgY, 180, 15, Text.literal(""));
    ...
    private void updateLayout() {
        ...
        this.OP_BUTTONS.put(OperationButtonType.TYPE.getCode(), new OperationButton(OperationButtonType.TYPE.getCode(), context -> {
            ...
            IText text = this.inventoryMode ? IText.i18n("列出模式\n物品栏 (%s)", playerItemList.size()) : IText.i18n("列出模式\n所有物品 (%s)", allItemList.size());
        ...
        this.OP_BUTTONS.put(OperationButtonType.ITEM.getCode(), new OperationButton(OperationButtonType.ITEM.getCode(), context -> {
            ...
            context.button().setTooltip(AbstractGuiUtils.componentToText(((ItemStack) RewardManager.deserializeReward(this.currentItem)).getName().copy()));
                            ...
                            if (context.button().isHovered()) {
                                ...
                                this.visibleTags.forEach((itemITag) -> {
                                    if (itemStack.isIn(itemITag)) {
                                        list1.add(1, (Text.literal("#" + itemITag.id())).formatted(Formatting.DARK_PURPLE));
                                    }
                                });
    ...
    private void handleOperation(OperationButton bt, int button, AtomicBoolean flag, AtomicBoolean updateSearchResults) {
        ...
        // 编辑奖励Json
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.i18n("请输入物品Json").setShadow(true)
                    , IText.i18n("请输入")
                    , ""
        ...
        // 编辑数量
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.i18n("请输入物品数量").setShadow(true)
                    , IText.i18n("请输入")
                    , "\\d{0,4}"
        ...
        // 编辑NBT
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.i18n("请输入物品NBT").setShadow(true)
                    , IText.i18n("请输入")
                    , ""
                    , itemNbtJsonString
                    , input -> {

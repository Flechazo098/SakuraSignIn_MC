...
import com.flechazo.util.TextureUtils;
import lombok.Data;
...
public class OperationButton {
     ...
     */
    public record RenderContext(DrawContext graphics, double mouseX, double mouseY, OperationButton button) {
    }
     ...
     * @param resource  资源
     */
    public OperationButton(int operation, Identifier resource) {
        this.operation = operation;
    ...
    public boolean isMouseOverEx(double mouseX, double mouseY) {
        ...
        if (image != null) {
            int pixel = image.getColor(textureX, textureY);
    ...
    public OperationButton setTooltip(String content) {
        this.tooltip = IText.literal(content);
        return this;
    }

    public OperationButton setTooltip(IText text) {
        this.tooltip = text;
        return this;
    }
     ...
     */
    public void render(DrawContext graphics, double mouseX, double mouseY, boolean renderPopup, int keyCode, int modifiers) {
     ...
     * 绘制弹出层
     */
    public void renderPopup(DrawContext graphics, double mouseX, double mouseY) {
     ...
     * 绘制弹出层
     */
    public void renderPopup(DrawContext graphics, TextRenderer font, double mouseX, double mouseY) {
     ...
     * 绘制弹出层
     */
    public void renderPopup(DrawContext graphics, double mouseX, double mouseY, int keyCode, int modifiers) {
        this.renderPopup(graphics, null, mouseX, mouseY, keyCode, modifiers);
    }
     ...
     * 绘制弹出层
     */
    public void renderPopup(DrawContext graphics, TextRenderer font, double mouseX, double mouseY, int keyCode, int modifiers) {
        // 绘制提示
        if (this.keyCode == -1 || (this.keyCode == keyCode && this.modifiers == modifiers)) {
            if (this.isHovered()) {
                if (customPopupFunction != null) {
                    customPopupFunction.run();
                } else if (tooltip != null && StringUtils.isNotNullOrEmpty(tooltip.getContent()) && MinecraftClient.getInstance().currentScreen != null) {
                    if (font == null) font = MinecraftClient.getInstance().textRenderer;
                    AbstractGuiUtils.drawPopupMessage(tooltip.setGraphics(graphics).setFont(font), (int) mouseX, (int) mouseY, MinecraftClient.getInstance().currentScreen.width, MinecraftClient.getInstance().currentScreen.height);
                }
            }
        }
    }

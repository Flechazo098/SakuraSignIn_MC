...
public class PopupOption {
    ...
    private final List <IText> optionList = new ArrayList <> ();
    @Getter
    private final List<IText> renderList = new ArrayList<>();
     ...
     * 提示文字
     */
    private final Map<Integer, IText> tipsMap = new HashMap<>();
    /**
     * 提示文字是否仅按下按键时显示
     */
    @Setter
    ...
    private PopupOption(TextRenderer textRenderer) {
    ...
    private PopupOption calculateSize() {
        ...
        this.screenWidth = MinecraftClient.getInstance().currentScreen.width;
        this.screenHeight = MinecraftClient.getInstance().currentScreen.height;
        // 计算弹出层的宽度和高度
        ...
        this.width = Math.min(AbstractGuiUtils.getTextWidth(this.textRenderer, renderList) + this.leftPadding + this.rightPadding, this.maxWidth);
        this.height = Math.min(AbstractGuiUtils.getTextHeight(this.textRenderer, renderList) + renderList.size() - 1 + this.topPadding + this.bottomPadding, this.maxHeight);
        // 计算弹出层的最大行数
        this.maxLines = ((this.height - this.topPadding - this.bottomPadding) + 1) / (this.textRenderer.fontHeight + 1);
        // 初始化调整后的坐标
    ...
    public static PopupOption init(TextRenderer font) {
        return new PopupOption(font);
    }

    public static PopupOption init(TextRenderer font, int x, int y) {
        return new PopupOption(font).setPosition(x, y).calculateSize();
    }

    public PopupOption addOption(@NonNull IText text) {
        ...
        List<IText> renderList = Arrays.stream(StringUtils.replaceLine(text.getContent()).split("\n")).map(s -> text.copy().setText(s).setHoverText(s)).toList();
    ...
    public PopupOption addOption(@NonNull String... text) {
        if (this.x >= 0 || this.y >= 0)
            throw new RuntimeException("The addOption method must be called after the clear/init method and before the resize method.");
        for (String s : text) {
            IText literal = IText.literal(s);
            List<IText> renderList = Arrays.stream(StringUtils.replaceLine(s).split("\n")).map(IText::literal).toList();
            for (int i = 0; i < renderList.size(); i++) {
                this.relationMap.put(this.renderList.size() + i, optionList.size());
            }
    ...
    public PopupOption addTips(@NonNull IText text, int index) {
        if (this.x >= 0 || this.y >= 0)
    ...
    public PopupOption addTips(@NonNull String text, int index) {
        if (this.x >= 0 || this.y >= 0)
            throw new RuntimeException("The addTips method must be called after the clear/init method and before the resize method.");
        tipsMap.put(index, IText.literal(text));
        return this;
    }

    public PopupOption addTips(@NonNull IText text) {
        if (this.x >= 0 || this.y >= 0)
            throw new RuntimeException("The addTips method must be called after the clear/init method and before the resize method.");
    ...
    public PopupOption addTips(@NonNull String... text) {
        ...
        for (int i = 0; i < text.length; i++) {
            tipsMap.put(index + i, IText.literal(text[i]));
        }
        return this;
    }
    ...
    public PopupOption setMaxLines(int maxLines) {
        ...
        this.maxHeight = maxLines * (this.textRenderer.fontHeight + 1) + this.topPadding + this.bottomPadding - 1;
        return this;
    }
     ...
     * @param id   标识
     */
    public PopupOption build(TextRenderer font, double x, double y, String id) {
    ...
    public void render(DrawContext graphics, double mouseX, double mouseY, int keyCode, int modifiers) {
            ...
            if (this.adjustedX < mouseX && mouseX < this.adjustedX + this.width - 1) {
                if (relativeY >= 0 && relativeY <= height) {
                    ...
                    for (int i = 0; i < (this.maxLines > 0 ? this.maxLines : renderList.size()); i++) {
                        ...
                        if (relativeY >= lines * (textRenderer.fontHeight + 1) && relativeY < (lines + curLines) * (textRenderer.fontHeight + 1) - 1 && relativeY < this.height - this.topPadding - this.bottomPadding) {
        ...
        for (int i = 0; i < this.maxLines; i++) {
            ...
            if (index >= 0 && index < renderList.size()) {
                IText text = renderList.get(index);
                if (selectedIndex == index) {
                    AbstractGuiUtils.fill(graphics, adjustedX + 1, adjustedY + topPadding + (lineOffset * (this.textRenderer.fontHeight + 1)), width - 2, this.textRenderer.fontHeight * StringUtils.getLineCount(text.getContent()), 0x88ACACAC);
                }
                if (maxWidth > 0) {
                    AbstractGuiUtils.drawLimitedText(text.setGraphics(graphics).setFont(this.textRenderer), adjustedX + leftPadding, adjustedY + topPadding + (i * (this.textRenderer.fontHeight + 1)), maxWidth, AbstractGuiUtils.EllipsisPosition.MIDDLE);
                } else {
                    AbstractGuiUtils.drawString(text.setGraphics(graphics).setFont(this.textRenderer), adjustedX + leftPadding, adjustedY + topPadding + (i * (this.textRenderer.fontHeight + 1)));
                }
        ...
        if (this.tipsKeyCode == -1 || (this.tipsKeyCode == keyCode && this.tipsModifiers == modifiers)) {
            if (this.getSelectedIndex() >= 0 && !tipsMap.isEmpty()) {
                IText text = tipsMap.getOrDefault(this.getSelectedIndex(), IText.literal(""));
                if (StringUtils.isNullOrEmpty(text.getContent())) {
                    text = tipsMap.getOrDefault(this.getSelectedIndex() - renderList.size(), IText.literal(""));
                }
                if (StringUtils.isNotNullOrEmpty(text.getContent())) {
                    AbstractGuiUtils.drawPopupMessage(text.setGraphics(graphics).setFont(this.textRenderer), (int) mouseX, (int) mouseY, this.screenWidth, this.screenHeight);
                }
            }
        }

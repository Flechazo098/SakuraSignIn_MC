...
import com.flechazo.sakura.config.RewardOptionData;
...
import com.flechazo.sakura.rewards.impl.*;
...
public class RewardManager {
                ...
                .collect(Collectors.groupingBy(reward -> {
                            ...
                            switch (type) {
                                case ITEM:
                                    ...
                                    key = Registries.ITEM.getKey(itemStack.getItem()).toString();
                                    if (itemStack.hasNbt()) {
                        ...
                        Collectors.reducing(null, (reward1, reward2) -> {
                            ...
                            switch (type) {
                                case ITEM:
                                    ...
                                    ((ItemStack) content1).setNbt(((ItemStack) content2).getNbt());
                                    break;
                                case EFFECT:
                                    content1 = new StatusEffectInstance(((StatusEffectInstance) content1).getEffectType(), ((StatusEffectInstance) content1).getDuration() + ((StatusEffectInstance) content2).getDuration(), ((StatusEffectInstance) content1).getAmplifier());
                                    break;
                                case EXP_POINT:
                                case SIGN_IN_CARD:
                                case EXP_LEVEL:
    ...
    public static void signIn(ServerPlayerEntity player, SignInPacket packet) {
        ...
        if (ESignInType.REWARD.equals(packet.getSignInType())) {
            if (isRewarded(signInData, signCompensateDate, false)) {
                ...
                boolean showFailed = player.hasPermissionLevel(ServerConfig.getPERMISSION_REWARD_FAILED_TIPS());
                        ...
                        .forEach(record -> {
                            ...
                            record.getRewardList().stream()
                                    ...
                                    .forEach(reward -> {
                                        reward.setDisabled(true);
                                        reward.setRewarded(true);
                                        Component detail = Component.literal(reward.getName(SakuraUtils.getClientLanguage(), true));
    ...
    public static boolean giveRewardToPlayer(ServerPlayerEntity player, IPlayerSignInData signInData, Reward reward) {
        ...
        if (ServerConfig.getREWARD_AFFECTED_BY_LUCK()) {
            int offset = player.getStatusEffects().stream()
                    ...
                    .map(instance -> {
                        if (instance.getEffectType() == StatusEffects.LUCK) {
        ...
        switch (reward.getType()) {
            ...
            case SIGN_IN_CARD:
                ...
                break;
            case EFFECT:
                player.addStatusEffect((StatusEffectInstance) object);
                break;
            case EXP_LEVEL:
                player.addExperienceLevels((Integer) object);
                break;
            case EXP_POINT:
                player.addExperience((Integer) object);
                break;
            case ADVANCEMENT:
                Advancement advancement = player.server.getAdvancementLoader().get((Identifier) object);
                if (advancement != null) {
            ...
            case COMMAND:
                ...
                if (StringUtils.isNotNullOrEmpty(command)) {
                    player.server.getCommandManager().executeWithPrefix(player.getCommandSource().withSilent().withLevel(2), command);
                }
                break;
            default:
        }
    ...
    public static boolean giveItemStack(ServerPlayerEntity player, ItemStack itemStack, boolean drop) {
        ...
        if (!added && drop) {
            ...
            try (World level = player.getWorld()) {
                added = level.spawnEntity(itemEntity);
            } catch (IOException e) {

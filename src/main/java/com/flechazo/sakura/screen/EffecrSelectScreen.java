...
import com.flechazo.sakura.rewards.impl.EffectRewardParser;
...
public class EffecrSelectScreen extends Screen {
    ...
    protected void init() {
        ...
        this.updateLayout();
        // 创建文本输入框
        this.inputField = AbstractGuiUtils.newTextFieldWidget(this.textRenderer, bgX, bgY, 112, 15, Component.empty());
    ...
    private void updateLayout() {
        ...
        this.OP_BUTTONS.put(OperationButtonType.TYPE.getCode(), new OperationButton(OperationButtonType.TYPE.getCode(), context -> {
            ...
            IText text = IText.translatable(EI18nType.TIPS, (this.playerMode ? "effect_select_list_player_mode" : "effect_select_list_all_mode"), (this.playerMode ? playerMobEffectList.size() : allMobEffectList.size()));
        ...
        this.OP_BUTTONS.put(OperationButtonType.MOBEFFECT.getCode(), new OperationButton(OperationButtonType.MOBEFFECT.getCode(), context -> {
            ...
            context.button().setTooltip(IText.fromTextComponent(((StatusEffectInstance) RewardManager.deserializeReward(this.currentMobEffect)).getEffectType().getName().copy()));
        ...
        this.OP_BUTTONS.put(OperationButtonType.DURATION.getCode(), new OperationButton(OperationButtonType.DURATION.getCode(), context -> {
            ...
            context.graphics().drawItem(itemStack, (int) context.button().getX() + 2, (int) context.button().getY() + 2);
            IText text = IText.translatable(EI18nType.TIPS, "set_duration_s", ((StatusEffectInstance) RewardManager.deserializeReward(this.currentMobEffect)).getDuration());
            context.button().setTooltip(text);
        ...
        this.OP_BUTTONS.put(OperationButtonType.AMPLIFIER.getCode(), new OperationButton(OperationButtonType.AMPLIFIER.getCode(), context -> {
            ...
            context.graphics().drawItem(itemStack, (int) context.button().getX() + 2, (int) context.button().getY() + 2);
            IText text = IText.translatable(EI18nType.TIPS, "set_amplifier_s", StringUtils.intToRoman(((StatusEffectInstance) RewardManager.deserializeReward(this.currentMobEffect)).getAmplifier() + 1));
            context.button().setTooltip(text);
    ...
    private void handleOperation(OperationButton bt, int button, AtomicBoolean flag, AtomicBoolean updateSearchResults) {
        ...
        // 编辑效果Json
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.translatable(EI18nType.TIPS, "enter_effect_json").setShadow(true)
        ...
        // 编辑持续时间
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.translatable(EI18nType.TIPS, "enter_effect_duration").setShadow(true)
                    , IText.translatable(EI18nType.TIPS, "enter_something")
                    , "\\d{0,4}"
        ...
        // 编辑效果等级
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.translatable(EI18nType.TIPS, "enter_effect_amplifier").setShadow(true)
                    , IText.translatable(EI18nType.TIPS, "enter_something")
                    , ""

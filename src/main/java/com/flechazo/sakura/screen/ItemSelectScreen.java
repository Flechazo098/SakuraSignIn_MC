...
import com.flechazo.sakura.config.ArraySet;
import com.flechazo.sakura.config.StringList;
...
import com.flechazo.sakura.rewards.impl.ItemRewardParser;
import com.flechazo.sakura.screen.component.IText;
import com.flechazo.sakura.screen.component.OperationButton;
...
public class ItemSelectScreen extends Screen {
    ...
    private final DefaultedList <ItemStack> allItemList = this.getAllItemList();
    ...
    protected void init() {
        ...
        this.updateLayout();
        // 创建文本输入框
        this.inputField = AbstractGuiUtils.newTextFieldWidget(this.textRenderer, bgX, bgY, 180, 15, Component.empty());
        this.inputField.setText(this.inputFieldText);
        this.addDrawableChild(this.inputField);
        // 创建提交按钮
        this.addDrawableChild (AbstractGuiUtils.newButton ((int) (this.bgX + 90 + this.margin), (int) (this.bgY + (20 + (AbstractGuiUtils.ITEM_ICON_SIZE + 3) * 5 + margin))
        ...
        // 创建取消按钮
        this.addDrawableChild(AbstractGuiUtils.newButton((int) (this.bgX + this.margin), (int) (this.bgY + (20 + (AbstractGuiUtils.ITEM_ICON_SIZE + 3) * 5 + margin))
                , (int) (90 - this.margin * 2), 20
    ...
    private DefaultedList<ItemStack> getAllItemList() {
        ...
        list.addAll(ItemGroups.getSearchGroup().getDisplayStacks());
        return list;
    }
    ...
    private void updateLayout() {
        ...
        this.OP_BUTTONS.put(OperationButtonType.TYPE.getCode(), new OperationButton(OperationButtonType.TYPE.getCode(), context -> {
            ...
            IText text = IText.translatable(EI18nType.TIPS, (this.inventoryMode ? "item_select_list_inventory_mode" : "item_select_list_all_mode"), (this.inventoryMode ? playerItemList.size() : allItemList.size()));
        ...
        this.OP_BUTTONS.put(OperationButtonType.ITEM.getCode(), new OperationButton(OperationButtonType.ITEM.getCode(), context -> {
            ...
            AbstractGuiUtils.fill(context.graphics(), (int) context.button().getX(), (int) context.button().getY(), (int) context.button().getWidth(), (int) context.button().getHeight(), 0xEE707070, 2);
            AbstractGuiUtils.fillOutLine(context.graphics(), (int) context.button().getX(), (int) context.button().getY(), (int) context.button().getWidth(), (int) context.button().getHeight(), 1, lineColor, 2);
            context.graphics().drawItem(RewardManager.deserializeReward(this.currentItem), (int) context.button().getX() + 2, (int) context.button().getY() + 2);
            context.button().setTooltip(IText.fromTextComponent(((ItemStack) RewardManager.deserializeReward(this.currentItem)).getName().copy()));
        ...
        this.OP_BUTTONS.put(OperationButtonType.COUNT.getCode(), new OperationButton(OperationButtonType.COUNT.getCode(), context -> {
            ...
            context.graphics().drawItem(itemStack, (int) context.button().getX() + 2, (int) context.button().getY() + 2);
            IText text = IText.translatable(EI18nType.TIPS, "set_count_s", ((ItemStack) RewardManager.deserializeReward(this.currentItem)).getCount());
            context.button().setTooltip(text);
        ...
        this.OP_BUTTONS.put(OperationButtonType.NBT.getCode(), new OperationButton(OperationButtonType.NBT.getCode(), context -> {
            ...
            context.graphics().drawItem(itemStack, (int) context.button().getX() + 2, (int) context.button().getY() + 2);
            IText text = IText.translatable(EI18nType.TIPS, "edit_nbt");
            context.button().setTooltip(text);
        ...
        this.OP_BUTTONS.put(OperationButtonType.PROBABILITY.getCode(), new OperationButton(OperationButtonType.PROBABILITY.getCode(), context -> {
            ...
            IText text = IText.translatable(EI18nType.TIPS, "set_probability_f", this.probability.multiply(new BigDecimal(100)).floatValue());
            context.button().setTooltip(text);
            ...
            for (int j = 0; j < itemPerLine; j++) {
                ITEM_BUTTONS.add(new OperationButton(itemPerLine * i + j, context -> {
                    ...
                    if (index >= 0 && index < itemList.size()) {
                        ...
                        context.graphics().drawItem(itemStack, (int) context.button().getX() + 1, (int) context.button().getY() + 1);
                        // 绘制物品详情悬浮窗
                        context.button().setCustomPopupFunction(() -> {
                            if (context.button().isHovered()) {
                                List< Text > list = itemStack.getTooltip(MinecraftClient.getInstance().player, MinecraftClient.getInstance().options.advancedItemTooltips ? TooltipContext.Default.ADVANCED : TooltipContext.Default.BASIC);
                                List< Text > list1 = Lists.newArrayList(list);
                                Item item = itemStack.getItem();
                                this.visibleTags.forEach((itemITag) -> {
                                    if (itemStack.isIn(itemITag)) {
                                        list1.add(1, (Component.literal("#" + itemITag.id())).setColor(Formatting.DARK_PURPLE.getColorValue()).toTextComponent());
                                    }
                                });
                                for (ItemGroup modeTab : ItemGroups.getGroups()) {
                                    if (modeTab.contains(itemStack)) {
                                        list1.add(1, modeTab.getDisplayName().copy().formatted(Formatting.BLUE));
                                    }
                                }

                                // 获取 TooltipData 数据
                                Optional< TooltipData > tooltipData = itemStack.getTooltipData();

                                context.graphics().drawTooltip(textRenderer, list1, tooltipData, (int) context.mouseX(), (int) context.mouseY());
                            }
                        });
                    } else {
    ...
    private void updateSearchResults() {
        ...
        if (StringUtils.isNotNullOrEmpty(s)) {
            ...
            if (s.startsWith("#")) {
                ...
                this.itemList.addAll(MinecraftClient.getInstance().getSearchProvider(SearchManager.ITEM_TAG).findAll(s.toLowerCase(Locale.ROOT)));
            }
            // $ 描述
                ...
                this.itemList.addAll(MinecraftClient.getInstance().getSearchProvider(SearchManager.ITEM_TOOLTIP).findAll(s.toLowerCase(Locale.ROOT)));
            }
        } else {
    ...
    private void updateVisibleTags(String string) {
        ...
        Registries.ITEM.streamTags().filter((tagKey) -> predicate.test(tagKey.id())).forEach(this.visibleTags::add);
    }

    private List<ItemStack> searchByDescription(String keyword) {
        return this.getItemList().stream()
                .filter(item -> item.getTooltip(MinecraftClient.getInstance().player, TooltipContext.Default.ADVANCED)
    ...
    private void handleOperation(OperationButton bt, int button, AtomicBoolean flag, AtomicBoolean updateSearchResults) {
        ...
        // 编辑奖励Json
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.translatable(EI18nType.TIPS, "enter_item_json").setShadow(true)
        ...
        // 编辑数量
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.translatable(EI18nType.TIPS, "enter_item_count").setShadow(true)
                    , IText.translatable(EI18nType.TIPS, "enter_something")
                    , "\\d{0,4}"
        ...
        // 编辑NBT
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.translatable(EI18nType.TIPS, "enter_item_nbt").setShadow(true)
                    , IText.translatable(EI18nType.TIPS, "enter_something")
                    , ""
                    , itemNbtJsonString
                    , input -> {
        ...
        // 编辑概率
            ...
            MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                    , IText.translatable(EI18nType.TIPS, "enter_reward_probability").setShadow(true)
                    , IText.translatable(EI18nType.TIPS, "enter_something")
                    , "(0?1(\\.0{0,5})?|0(\\.\\d{0,5})?)?"

...
public class SignInCell {
    private final Identifier BACKGROUND_TEXTURE;
    ...
    public SignInCell(Identifier resourceLocation, TextureCoordinate textureCoordinate, double x, double y, double width, double height, double scale, @NonNull RewardList rewardList, int year, int month, int day, int status) {
    ...
    // 渲染格子
    public void render(DrawContext graphics, TextRenderer font, int mouseX, int mouseY) {
        ...
        if (showText) {
            ...
            float dayWidth = font.getWidth(dayComponent.toString());
            graphics.drawText(font, dayComponent.setColor(color).toTextComponent(), (int) (x + (width - dayWidth) / 2), (int) (y + textureCoordinate.getDateOffset() * this.scale + 0.1f), color, false);
        }
    }

    // 绘制奖励详情弹出层
    public void renderTooltip(DrawContext graphics, TextRenderer font, int mouseX, int mouseY) {
        // 禁用深度测试
        RenderSystem.disableDepthTest();
        graphics.getMatrices().push();
        // 提升Z坐标以确保弹出层在最上层
        graphics.getMatrices().translate(0, 0, 200.0F);
        ...
        if (MinecraftClient.getInstance().player != null) {
            showQuality = MinecraftClient.getInstance().player.hasPermissionLevel(ServerConfig.getPERMISSION_REWARD_PROBABILITY());
        }
        ...
        // 绘制文字
        String monthTitle = DateUtils.toLocalStringMonth(DateUtils.getDate(year, month, day), MinecraftClient.getInstance().options.language);
        String dayTitle = DateUtils.toLocalStringDay(DateUtils.getDate(year, month, day), MinecraftClient.getInstance().options.language);
        Component title = Component.literal(String.format("%s %s", monthTitle, dayTitle));
        double fontWidth = font.getWidth(title.toString());
        ...
        graphics.drawText(font, title.setColor(color).toTextComponent(), (int) tooltipDateX, (int) tooltipDateY, color, false);

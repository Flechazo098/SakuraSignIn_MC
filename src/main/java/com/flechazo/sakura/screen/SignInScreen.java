...
import com.flechazo.sakura.enums.ESignInStatus;
...
import com.flechazo.sakura.screen.component.PopupOption;
...
import static com.flechazo.sakura.screen.SignInScreen.OperationButtonType.*;
...
public class SignInScreen extends Screen{
    ...
    protected void init() {
        ...
        tips = IText.translatable(EI18nType.TIPS, "sign_in_screen_tips");
    ...
    private void updateButtons() {
        ...
        BUTTONS.put(LEFT_ARROW.getCode(), new OperationButton(LEFT_ARROW.getCode(), texture)
                ...
                .setTooltip(IText.translatable(EI18nType.TIPS, "use_s_key", "←"))
        ...
        BUTTONS.put(THEME_ORIGINAL_BUTTON.getCode(), new OperationButton(THEME_ORIGINAL_BUTTON.getCode(), texture)
                ...
                .setTextureWidth(textureCoordinate.getTotalWidth())
                .setTextureHeight(textureCoordinate.getTotalHeight())
                .setTooltip(IText.translatable(EI18nType.TIPS, "click_to_change_theme")));
        ...
        BUTTONS.put(THEME_CHAOS_BUTTON.getCode(), new OperationButton(THEME_CHAOS_BUTTON.getCode(), texture)
                ...
                .setTooltip(IText.translatable(EI18nType.TIPS, "click_to_change_theme_or_select_external_theme").setAlign(IText.Align.CENTER)));
    }
    ...
    private void createCalendarCells(Date current) {
        ...
        if (player != null) {
            ...
            if ((player.hasPermissionLevel(ServerConfig.getPERMISSION_REWARD_DETAIL()))) {
    ...
    public void render(DrawContext graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (!this.SIGN_IN_SCREEN_TIPS) {
            ...
            if (MinecraftClient.getInstance().player != null) {
                showRewardDetail = MinecraftClient.getInstance().player.hasPermissionLevel(ServerConfig.getPERMISSION_REWARD_DETAIL());
            }
            if (showRewardDetail) {
                // 渲染格子弹出层
                for (SignInCell cell : signInCells) {
                    if (cell.isShowHover() && cell.isMouseOver(mouseX, mouseY)) {
                        if ((this.keyCode == GLFW.GLFW_KEY_LEFT_SHIFT || this.keyCode == GLFW.GLFW_KEY_RIGHT_SHIFT) && this.modifiers == GLFW.GLFW_MOD_SHIFT) {
                            AbstractGuiUtils.drawPopupMessage(IText.translatable(EI18nType.TIPS, "how_to_sign_in").setGraphics(graphics).setFont(this.textRenderer).setAlign(IText.Align.CENTER), mouseX, mouseY, super.width, super.height);
                        } else {
            ...
            for (Integer op : BUTTONS.keySet()) {
                ...
                if (op == INFO.getCode()) {
                    if (MinecraftClient.getInstance().player != null) {
                        ...
                        button.setTooltip(
                                IText.translatable(EI18nType.TIPS, "sign_in_info"
            ...
            for (Element child : this.children()) {
                if (child instanceof ButtonWidget button) {
                    ...
                    boolean isConfirmButton = buttonText.equalsIgnoreCase(IText.translatable(EI18nType.OPTION, "confirm").getContent());
                    boolean isDontShowAgainButton = buttonText.equalsIgnoreCase(IText.translatable(EI18nType.OPTION, "no_remind").getContent());
    ...
    private void handleOperation(double mouseX, double mouseY, int button, OperationButton value, AtomicBoolean updateLayout, AtomicBoolean updateTexture, AtomicBoolean flag) {
        ...
        // 混沌主题
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                popupOption.setMaxWidth(AbstractGuiUtils.multilineTextWidth(IText.translatable(EI18nType.TITLE, "theme_selector_empty")))
                        .setMaxLines(5)
        ...
        if (cell.status == ESignInStatus.NOT_SIGNED_IN.getCode()) {
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                if (RewardManager.getCompensateDateInt() < DateUtils.toDateInt(RewardManager.getCompensateDate(DateUtils.getClientDate()))) {
                    ...
                    SignInPacket packet = new SignInPacket(DateUtils.toDateTimeString(DateUtils.getClientDate()), ClientConfig.getAUTO_REWARDED(), ESignInType.SIGN_IN);
                    packet.toBytes(buf);
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                if (RewardManager.isRewarded(PlayerSignInDataCapability.getData(player), cellDate, false)) {
                    ...
                    PacketByteBuf buf = PacketByteBufs.create();
                    SignInPacket packet = new SignInPacket(DateUtils.toDateTimeString(cellDate), ClientConfig.getAUTO_REWARDED(), ESignInType.REWARD);
                    packet.toBytes(buf);
                    ClientPlayNetworking.send(ModNetworkHandler.SIGN_IN, buf);
                }
            }
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
                if (! ServerConfig.getSIGN_IN_CARD()) {
                    ...
                    if (PlayerSignInDataCapability.getData(player).getSignInCard() <= 0) {
                        ...
                        cell.status = ClientConfig.getAUTO_REWARDED() ? ESignInStatus.REWARDED.getCode() : ESignInStatus.SIGNED_IN.getCode();
                        PacketByteBuf buf = PacketByteBufs.create();
                        SignInPacket packet = new SignInPacket(DateUtils.toDateTimeString(cellDate), ClientConfig.getAUTO_REWARDED(), ESignInType.RE_SIGN_IN);
                        packet.toBytes(buf);
                        ClientPlayNetworking.send(ModNetworkHandler.SIGN_IN, buf);
                    }
                }

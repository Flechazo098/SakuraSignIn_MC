...
import com.flechazo.sakura.config.*;
...
public class RewardOptionScreen extends Screen {
    ...
    public void renderBackgroundTexture (DrawContext graphics) {
        ...
        // 绑定背景纹理
        AbstractGuiUtils.bindTexture(SakuraSignInFabric.getThemeTexture());
        ...
        // 获取纹理指定区域的坐标和大小
        float u0 = (float) SakuraSignInFabric.getThemeTextureCoordinate().getOptionBgUV().getU0();
        float v0 = (float) SakuraSignInFabric.getThemeTextureCoordinate().getOptionBgUV().getV0();
        float regionWidth = (float) SakuraSignInFabric.getThemeTextureCoordinate().getOptionBgUV().getUWidth();
        float regionHeight = (float) SakuraSignInFabric.getThemeTextureCoordinate().getOptionBgUV().getVHeight();
        if (regionWidth == 0) regionWidth = screenWidth;
        if (regionHeight == 0) regionHeight = screenHeight;
        int textureTotalWidth = SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth();
        int textureTotalHeight = SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight();
        ...
        for (int x = 0; x <= screenWidth - regionWidth; x += (int) regionWidth) {
            for (int y = 0; y <= screenHeight - regionHeight; y += (int) regionHeight) {
                ...
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x, y + regionHeight, 0).texture(uMin, vMax).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x + regionWidth, y + regionHeight, 0).texture(uMax, vMax).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x + regionWidth, y, 0).texture(uMax, vMin).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x, y, 0).texture(uMin, vMin).next();
                tesselator.draw();
            }
        }
        ...
        if (leftoverWidth > 0) {
            for (int y = 0; y <= screenHeight - regionHeight; y += (int) regionHeight) {
                buffer.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE);
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth - leftoverWidth, y + regionHeight, 0).texture(uMin, vMax).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth, y + regionHeight, 0).texture(u, vMax).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth, y, 0).texture(u, vMin).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth - leftoverWidth, y, 0).texture(uMin, vMin).next();
                tesselator.draw();
            }
        }
        ...
        if (leftoverHeight > 0) {
            for (int x = 0; x <= screenWidth - regionWidth; x += (int) regionWidth) {
                buffer.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE);
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x, screenHeight, 0).texture(uMin, v).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x + regionWidth, screenHeight, 0).texture(uMax, v).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x + regionWidth, screenHeight - leftoverHeight, 0).texture(uMax, vMin).next();
                buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), x, screenHeight - leftoverHeight, 0).texture(uMin, vMin).next();
                tesselator.draw();
            }
        }
        ...
        if (leftoverWidth > 0 && leftoverHeight > 0) {
            buffer.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE);
            buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth - leftoverWidth, screenHeight, 0).texture(uMin, v).next();
            buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth, screenHeight, 0).texture(u, v).next();
            buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth, screenHeight - leftoverHeight, 0).texture(u, vMin).next();
            buffer.vertex(graphics.getMatrices().peek().getPositionMatrix(), screenWidth - leftoverWidth, screenHeight - leftoverHeight, 0).texture(uMin, vMin).next();
            tesselator.draw();
        }
    ...
    private void addRewardTitleButton(String title, String key, int titleIndex, int index) {
        REWARD_BUTTONS.put(String.format("标题,%s", key), new OperationButton(titleIndex, context -> {
            if (context.button().getRealY() < super.height && context.button().getRealY() + context.button().getRealHeight() >= 0) {
                ...
                AbstractGuiUtils.drawLimitedText(this.gg, super.textRenderer, title, (int) context.button().getRealX(), (int) (context.button().getRealY() + (context.button().getRealHeight() - super.textRenderer.fontHeight) / 2), (int) context.button().getRealWidth(), 0xAC000000, false);
                context.graphics().fill((int) context.button().getRealX(), (int) (context.button().getRealY() + context.button().getRealHeight()), (int) (context.button().getRealX() + super.textRenderer.getWidth(title)), (int) (context.button().getRealY() + context.button().getRealHeight() - 1), 0xAC000000);
            }
        })
                .setX(leftMargin)
        ...
        for (int j = 0; j < rewardMap.get(key).size(); j++, index.incrementAndGet()) {
            REWARD_BUTTONS.put(String.format("%s,%s", key, j), new OperationButton(j, context -> {
                if (context.button().getRealY() < super.height && context.button().getRealY() + context.button().getRealHeight() >= 0) {
                    ...
                    AbstractGuiUtils.renderCustomReward(this.gg, super.textRenderer, SakuraSignInFabric.getThemeTexture(), SakuraSignInFabric.getThemeTextureCoordinate(), reward, (int) context.button().getRealX(), (int) context.button().getRealY(), true);
                }
            })
    ...
    private StringInputScreen getRuleKeyInputScreen(Screen callbackScreen, ERewardRule rule, String[] key) {
        ...
        return new StringInputScreen(callbackScreen, IText.translatable(EI18nType.TIPS, "enter_reward_rule_key").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_something"), validator, "", input -> {
    ...
    private StringInputScreen getCdkRuleKeyInputScreen(Screen callbackScreen, ERewardRule rule, String[] key) {
        return new StringInputScreen(callbackScreen
                , new TextList(IText.translatable(EI18nType.TIPS, "enter_reward_rule_key").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_valid_until").setShadow(true))
                , new TextList(IText.translatable(EI18nType.TIPS, "enter_something"))
                , new StringList("\\w*", "")
     ...
     * 渲染奖励列表
     */
    private void renderRewardList(DrawContext graphics, double mouseX, double mouseY) {
    ...
    private final Consumer <PopupOption> pasteConsumer = option -> {
        ...
        if (paste.equalsIgnoreCase(option.getSelectedString())) {
            option.getRenderList().stream()
                    ...
                    .forEach(item -> item.setColor(!SakuraSignInFabric.getClipboard().isEmpty() ? 0xFFFFFFFF : 0xFF999999));
        }
    };
    ...
    private void handleOperation(double mouseX, double mouseY, int button, OperationButton value, AtomicBoolean updateLayout, AtomicBoolean flag) {
        ...
        if (value.getOperation() == OperationButtonType.OPEN.getCode()) {
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                SakuraSignInFabric.setRewardOptionBarOpened(true);
        ...
        // 关闭左侧边栏
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                SakuraSignInFabric.setRewardOptionBarOpened(false);
                updateLayout.set(true);
                flag.set(true);
            }
        }
        // 左侧边栏奖励规则类型按钮
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                ...
                try {
                    ...
                    if (!player.hasPermissionLevel(SakuraUtils.getRewardPermissionLevel(rewardRule))) {
                ...
                this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "clear").setColor(0xFFFF0000))
                        .addTips(IText.translatable(EI18nType.TIPS, "cancel_or_confirm"))
                        .setTipsKeyCode(GLFW.GLFW_KEY_LEFT_SHIFT)
                        .setTipsModifiers(GLFW.GLFW_MOD_SHIFT)
                        .build(super.textRenderer, mouseX, mouseY, String.format("奖励规则类型按钮:%s", value.getOperation()));
            }
        }
        // 重置偏移量
        ...
        // 奖励配置列表面板
            ...
            if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
                if (this.currOpButton > 200 && this.currOpButton <= 299) {
                    this.popupOption.clear();
                    this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "paste"));
                    for (ERewardType rewardType : ERewardType.values()) {
                        this.popupOption.addOption(IText.translatable(EI18nType.WORD, "reward_type_" + rewardType.getCode()));
                    }
                    this.popupOption.build(super.textRenderer, mouseX, mouseY, String.format("奖励面板按钮:%s", this.currOpButton));
        ...
        // 帮助按钮
            ...
            // 绘制弹出层提示
            this.popupOption.clear();
            this.popupOption.addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_1"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_2"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_3"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_4"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_5"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_6"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_7"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_8"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_9"))
                    .addOption(IText.translatable(EI18nType.TIPS, "reward_rule_description_10"))
                    .build(super.textRenderer, mouseX, mouseY, "reward_rule_description");
        }
        // 上传奖励配置
        else if (value.getOperation() == OperationButtonType.UPLOAD.getCode()) {
            // 仅管理员可上传
            ClientPlayerEntity player = MinecraftClient.getInstance().player;
            if (player != null && player.hasPermissionLevel(ServerConfig.getPERMISSION_EDIT_REWARD())) {
                for (RewardOptionSyncPacket rewardOptionSyncPacket : RewardOptionDataManager.toSyncPacket(player).split()) {
                    PacketByteBuf buf = PacketByteBufs.create();
                    rewardOptionSyncPacket.write(buf);
        ...
        if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
            if (key.startsWith("标题")) {
                ...
                if (!"标题,base".equalsIgnoreCase(key)) {
                    this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "edit"));
                }
                this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "copy"));
                if (!"标题,base".equalsIgnoreCase(key)) {
                    this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "cut"));
                }
                this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "paste"));
                for (ERewardType rewardType : ERewardType.values()) {
                    this.popupOption.addOption(IText.translatable(EI18nType.WORD, "reward_type_" + rewardType.getCode()));
                ...
                this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "clear").setColor(0xFFFF0000));
                if (!"标题,base".equalsIgnoreCase(key)) {
                    this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "delete").setColor(0xFFFF0000));
                }
                this.popupOption.addTips(IText.translatable(EI18nType.TIPS, "cancel_or_confirm"), -1)
                        .addTips(IText.translatable(EI18nType.TIPS, "cancel_or_confirm"), -2)
                        .setTipsKeyCode(GLFW.GLFW_KEY_LEFT_SHIFT)
                        .setTipsModifiers(GLFW.GLFW_MOD_SHIFT)
                        .build(super.textRenderer, mouseX, mouseY, String.format("奖励按钮:%s", key));
            } else {
                this.popupOption.clear();
                this.popupOption.addOption(IText.translatable(EI18nType.OPTION, "edit"))
                        .addOption(IText.translatable(EI18nType.OPTION, "copy"))
                        .addOption(IText.translatable(EI18nType.OPTION, "cut"))
                        .addOption(IText.translatable(EI18nType.OPTION, "paste"))
                        .addOption(IText.translatable(EI18nType.OPTION, "delete").setColor(0xFFFF0000))
                        .addTips(IText.translatable(EI18nType.TIPS, "cancel_or_confirm"), -1)
                        .setTipsKeyCode(GLFW.GLFW_KEY_LEFT_SHIFT)
                        .setTipsModifiers(GLFW.GLFW_MOD_SHIFT)
                        .build(super.textRenderer, mouseX, mouseY, String.format("奖励按钮:%s", key));
            }
    ...
    private void handlePopupOption(double mouseX, double mouseY, int button, AtomicBoolean updateLayout, AtomicBoolean flag) {
        ...
        if (popupOption.getId().startsWith("奖励规则类型按钮:")) {
            ...
            if (Component.translatableClient(EI18nType.OPTION, "paste").toString().equalsIgnoreCase(selectedString)) {
                if (!SakuraSignInFabric.getClipboard().isEmpty()) {
                    if (rule == ERewardRule.CDK_REWARD) {
                        MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                            ...
                            if (CollectionUtils.isNotNullOrEmpty(input)) {
                                ...
                                if (result.stream().allMatch(StringUtils::isNullOrEmptyEx)) {
                                    RewardList rewardList = SakuraSignInFabric.getClipboard().clone();
                        ...
                        MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                                , IText.translatable(EI18nType.TIPS, "enter_reward_rule_key").setShadow(true)
                                , IText.translatable(EI18nType.TIPS, "enter_something")
                                , validator
                                , ""
                                , input -> {
                            StringList result = new StringList();
            ...
            // 物品
                ...
                if (rule == ERewardRule.CDK_REWARD) {
                    MinecraftClient.getInstance().setScreen(this.getCdkRuleKeyInputScreen(callbackScreen, rule, key));
                } else if (rule != ERewardRule.BASE_REWARD) {
                    MinecraftClient.getInstance().setScreen(this.getRuleKeyInputScreen(callbackScreen, rule, key));
                } else {
                    key[0] = "base";
                    MinecraftClient.getInstance().setScreen(callbackScreen);
                }
            }
            // 药水效果
                ...
                EffecrSelectScreen callbackScreen = new EffecrSelectScreen(this, input -> {
                    if (input != null && ((StatusEffectInstance) RewardManager.deserializeReward(input)).getDuration() > 0 && StringUtils.isNotNullOrEmpty(key[0])) {
                        RewardOptionDataManager.addReward(rule, key[0], input);
                        RewardOptionDataManager.saveRewardOption();
                    }
                }, new Reward(new StatusEffectInstance(StatusEffects.LUCK), ERewardType.EFFECT), () -> StringUtils.isNullOrEmpty(key[0]));
                if (rule == ERewardRule.CDK_REWARD) {
                    MinecraftClient.getInstance().setScreen(this.getCdkRuleKeyInputScreen(callbackScreen, rule, key));
                } else if (rule != ERewardRule.BASE_REWARD) {
                    MinecraftClient.getInstance().setScreen(this.getRuleKeyInputScreen(callbackScreen, rule, key));
            ...
            // 经验点
                ...
                StringInputScreen callbackScreen = new StringInputScreen(this
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_exp_point").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_reward_probability").setShadow(true))
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_something"))
                ...
                StringInputScreen callbackScreen = new StringInputScreen(this
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_exp_level").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_reward_probability").setShadow(true))
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_something"))
                        , new StringList("-?\\d*", "(0?1(\\.0{0,5})?|0(\\.\\d{0,5})?)?")
                        , new StringList("1")
                        , input -> {
            ...
            // 补签卡
                ...
                StringInputScreen callbackScreen = new StringInputScreen(this
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_sign_in_card").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_reward_probability").setShadow(true))
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_something"))
                        , new StringList("-?\\d*", "(0?1(\\.0{0,5})?|0(\\.\\d{0,5})?)?")
                        , new StringList("1")
                        , input -> {
            ...
            // 进度
                ...
                }, new Reward(new Identifier(""), ERewardType.ADVANCEMENT), () -> StringUtils.isNullOrEmpty(key[0]));
                if (rule == ERewardRule.CDK_REWARD) {
                    MinecraftClient.getInstance().setScreen(this.getCdkRuleKeyInputScreen(callbackScreen, rule, key));
                } else if (rule != ERewardRule.BASE_REWARD) {
                    MinecraftClient.getInstance().setScreen(this.getRuleKeyInputScreen(callbackScreen, rule, key));
            ...
            // 消息
                ...
                StringInputScreen callbackScreen = new StringInputScreen(this
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_message").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_reward_probability").setShadow(true))
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_something"))
            ...
            // 指令
                ...
                StringInputScreen callbackScreen = new StringInputScreen(this
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_command").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_reward_probability").setShadow(true))
                        , new TextList(IText.translatable(EI18nType.TIPS, "enter_something"))
                        , new StringList("", "(0?1(\\.0{0,5})?|0(\\.\\d{0,5})?)?")
                        , new StringList("", "1")
                        , input -> {
            ...
            if (id.startsWith("标题")) {
                ...
                if (Component.translatableClient(EI18nType.OPTION, "edit").toString().equalsIgnoreCase(selectedString)) {
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        if (rule == ERewardRule.CDK_REWARD) {
                            ...
                            String validator = rule == ERewardRule.RANDOM_REWARD ? "(0?1(\\.0{0,10})?|0(\\.\\d{0,10})?)?" : "[\\d +~/:.tT-]*";
                            MinecraftClient.getInstance().setScreen(new StringInputScreen(this, IText.translatable(EI18nType.TIPS, "enter_reward_rule_key").setShadow(true), IText.translatable(EI18nType.TIPS, "enter_something"), validator, key, input -> {
                                StringList result = new StringList();
                                if (CollectionUtils.isNotNullOrEmpty(input)) {
                                    if (RewardOptionDataManager.validateKeyName(rule, input.get(0))) {
                    ...
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        ...
                        SakuraSignInFabric.getClipboard().clear();
                        SakuraSignInFabric.getClipboard().addAll(rewardList);
                    }
                ...
                // 添加物品
                    ...
                    MinecraftClient.getInstance().setScreen(new ItemSelectScreen(this, input -> {
                ...
                // 药水效果
                    ...
                    MinecraftClient.getInstance().setScreen(new EffecrSelectScreen(this, input -> {
                        if (input != null && ((StatusEffectInstance) RewardManager.deserializeReward(input)).getDuration() > 0) {
                            RewardOptionDataManager.addReward(rule, key, input);
                            RewardOptionDataManager.saveRewardOption();
                        }
                    }, new Reward(new StatusEffectInstance(StatusEffects.LUCK), ERewardType.EFFECT)));
                }
                // 经验点
                else if (I18nUtils.getTranslationClient(EI18nType.WORD, "reward_type_" + ERewardType.EXP_POINT.getCode()).equalsIgnoreCase(selectedString)) {
                    MinecraftClient.getInstance().setScreen(new StringInputScreen(this
                ...
                // 进度
                    ...
                    MinecraftClient.getInstance().setScreen(new AdvancementSelectScreen(this, input -> {
                        if (input != null && StringUtils.isNotNullOrEmpty(((Identifier) RewardManager.deserializeReward(input)).toString())) {
                            RewardOptionDataManager.addReward(rule, key, input);
                            RewardOptionDataManager.saveRewardOption();
                        }
                    }, new Reward(new Identifier (""), ERewardType.ADVANCEMENT)));
                ...
                if (Component.translatableClient(EI18nType.OPTION, "edit").toString().equalsIgnoreCase(selectedString)) {
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        ...
                        // 进度
                            ...
                            MinecraftClient.getInstance().setScreen(new AdvancementSelectScreen(this, input -> {
                                if (input != null && StringUtils.isNotNullOrEmpty(((Identifier) RewardManager.deserializeReward(input)).toString()) && StringUtils.isNotNullOrEmpty(key)) {
                                    RewardOptionDataManager.updateReward(rule, key, Integer.parseInt(index), input);
                                    RewardOptionDataManager.saveRewardOption();
                                }
                            }, reward));
                    ...
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        Reward reward = RewardOptionDataManager.getReward(rule, key, Integer.parseInt(index)).clone();
                        SakuraSignInFabric.getClipboard().clear();
                        SakuraSignInFabric.getClipboard().add(reward);
                    }
                } else if (Component.translatableClient(EI18nType.OPTION, "cut").toString().equalsIgnoreCase(selectedString)) {
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        Reward reward = RewardOptionDataManager.getReward(rule, key, Integer.parseInt(index)).clone();
                    ...
                    if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
                        if (!SakuraSignInFabric.getClipboard().isEmpty()) {
                            for (Reward reward : SakuraSignInFabric.getClipboard().clone()) {
    ...
    private Consumer<OperationButton.RenderContext> generateCustomRenderFunction(String content) {
        return context -> {
            ...
            AbstractGuiUtils.drawLimitedText(context.graphics(), super.textRenderer, Component.translatableClient(EI18nType.WORD, content).toString(), realX + 4, (int) (realY + (realHeight - super.textRenderer.fontHeight) / 2), (int) (realWidth - 22), 0xFFEBD4B1);
        };
    }

    private void updateLayout() {
        this.leftBarWidth = SakuraSignInFabric.isRewardOptionBarOpened() ? 100 : 20;
    ...
    protected void init() {
        ...
        OP_BUTTONS.put(OperationButtonType.OPEN.getCode(), new OperationButton(OperationButtonType.OPEN.getCode(), SakuraSignInFabric.getThemeTexture())
                .setCoordinate(new Coordinate().setX(4).setY((super.height - 16) / 2.0).setWidth(16).setHeight(16))
                .setNormal(SakuraSignInFabric.getThemeTextureCoordinate().getArrowUV()).setHover(SakuraSignInFabric.getThemeTextureCoordinate().getArrowHoverUV()).setTap(SakuraSignInFabric.getThemeTextureCoordinate().getArrowTapUV())
                .setTextureWidth(SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth())
                .setTextureHeight(SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight())
                .setTransparentCheck(false)
                .setTooltip(Component.translatableClient(EI18nType.TIPS, "open_sidebar").toString()));
        OP_BUTTONS.put(OperationButtonType.CLOSE.getCode(), new OperationButton(OperationButtonType.CLOSE.getCode(), SakuraSignInFabric.getThemeTexture())
                .setCoordinate(new Coordinate().setX(80).setY((5 * 2 + super.textRenderer.fontHeight - 16) / 2.0).setWidth(16).setHeight(16))
                .setNormal(SakuraSignInFabric.getThemeTextureCoordinate().getArrowUV()).setHover(SakuraSignInFabric.getThemeTextureCoordinate().getArrowHoverUV()).setTap(SakuraSignInFabric.getThemeTextureCoordinate().getArrowTapUV())
                .setTextureWidth(SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth())
                .setTextureHeight(SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight())
        ...
        OP_BUTTONS.put(OperationButtonType.OFFSET_Y.getCode(), new OperationButton(OperationButtonType.OFFSET_Y.getCode(), context -> {
            AbstractGuiUtils.drawString(context.graphics(), super.textRenderer, "OY:", super.width - rightBarWidth + 1, super.height - textRenderer.fontHeight * 2 - 2, 0xFFACACAC);
            AbstractGuiUtils.drawLimitedText(context.graphics(), super.textRenderer, String.valueOf((int) yOffset), super.width - rightBarWidth + 1, super.height - textRenderer.fontHeight - 2, rightBarWidth, 0xFFACACAC);
        })
                .setX(super.width - rightBarWidth).setY(super.height - textRenderer.fontHeight * 2 - 2).setWidth(rightBarWidth).setHeight(textRenderer.fontHeight * 2 + 2)
                .setTransparentCheck(false));
        OP_BUTTONS.put(OperationButtonType.HELP.getCode(), new OperationButton(OperationButtonType.HELP.getCode(), SakuraSignInFabric.getThemeTexture())
                .setCoordinate(SakuraSignInFabric.getThemeTextureCoordinate().getHelpUV())
                .setNormal(SakuraSignInFabric.getThemeTextureCoordinate().getHelpUV()).setHover(SakuraSignInFabric.getThemeTextureCoordinate().getHelpUV()).setTap(SakuraSignInFabric.getThemeTextureCoordinate().getHelpUV())
                .setTextureWidth(SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth())
                .setTextureHeight(SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight())
                .setX(super.width - rightBarWidth + 1).setY(2).setWidth(18).setHeight(18)
                .setHoverFgColor(0xAA808080).setTapFgColor(0xAA808080)
                .setTransparentCheck(false));
        OP_BUTTONS.put(OperationButtonType.DOWNLOAD.getCode(), new OperationButton(OperationButtonType.DOWNLOAD.getCode(), SakuraSignInFabric.getThemeTexture())
                .setCoordinate(SakuraSignInFabric.getThemeTextureCoordinate().getDownloadUV())
                .setNormal(SakuraSignInFabric.getThemeTextureCoordinate().getDownloadUV()).setHover(SakuraSignInFabric.getThemeTextureCoordinate().getDownloadUV()).setTap(SakuraSignInFabric.getThemeTextureCoordinate().getDownloadUV())
                .setTextureWidth(SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth())
                .setTextureHeight(SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight())
        ...
        OP_BUTTONS.put(OperationButtonType.UPLOAD.getCode(), new OperationButton(OperationButtonType.UPLOAD.getCode(), SakuraSignInFabric.getThemeTexture())
                .setCoordinate(SakuraSignInFabric.getThemeTextureCoordinate().getUploadUV())
                .setNormal(SakuraSignInFabric.getThemeTextureCoordinate().getUploadUV()).setHover(SakuraSignInFabric.getThemeTextureCoordinate().getUploadUV()).setTap(SakuraSignInFabric.getThemeTextureCoordinate().getUploadUV())
                .setTextureWidth(SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth())
                .setTextureHeight(SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight())
                .setX(super.width - rightBarWidth + 1).setY(42).setWidth(18).setHeight(18)
                .setHoverFgColor(0xAA808080).setTapFgColor(0xAAA0A0A0)
                .setTransparentCheck(false));
        OP_BUTTONS.put(OperationButtonType.FOLDER.getCode(), new OperationButton(OperationButtonType.FOLDER.getCode(), SakuraSignInFabric.getThemeTexture())
                .setCoordinate(SakuraSignInFabric.getThemeTextureCoordinate().getFolderUV())
                .setNormal(SakuraSignInFabric.getThemeTextureCoordinate().getFolderUV()).setHover(SakuraSignInFabric.getThemeTextureCoordinate().getFolderUV()).setTap(SakuraSignInFabric.getThemeTextureCoordinate().getFolderUV())
                .setTextureWidth(SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth())
                .setTextureHeight(SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight())
        ...
        OP_BUTTONS.put(OperationButtonType.SORT.getCode(), new OperationButton(OperationButtonType.SORT.getCode(), SakuraSignInFabric.getThemeTexture())
                .setCoordinate(SakuraSignInFabric.getThemeTextureCoordinate().getSortUV())
                .setNormal(SakuraSignInFabric.getThemeTextureCoordinate().getSortUV()).setHover(SakuraSignInFabric.getThemeTextureCoordinate().getSortUV()).setTap(SakuraSignInFabric.getThemeTextureCoordinate().getSortUV())
                .setTextureWidth(SakuraSignInFabric.getThemeTextureCoordinate().getTotalWidth())
                .setTextureHeight(SakuraSignInFabric.getThemeTextureCoordinate().getTotalHeight())
        ...
        tips = IText.translatable(EI18nType.TIPS, "reward_option_screen_tips");
    }
    ...
    public void render(DrawContext graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        // 绘制左侧边栏列表标题
        if (SakuraSignInFabric.isRewardOptionBarOpened()) {
            graphics.drawCenteredTextWithShadow(super.textRenderer, Component.translatableClient(EI18nType.TITLE, "reward_rule_type").toString(), 4, 5, 0xFFACACAC);
            ...
            // 绘制其他按钮
                ...
                // 帮助按钮
                    ...
                    if (player != null && player.hasPermissionLevel(ServerConfig.getPERMISSION_EDIT_REWARD())) {
                        ...
                        button.setTooltip(IText.translatable(EI18nType.TIPS, "upload_reward_config_no_permission").setColor(0xFFFF0000))
                                .setHoverFgColor(0xAA808080).setTapFgColor(0xAA808080);
                    }
                }
                button.renderPopup(graphics, mouseX, mouseY, this.keyCode, this.modifiers);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
            if (this.previousScreen != null) MinecraftClient.getInstance().setScreen(this.previousScreen);
            else this.close();
            return true;
        } else {

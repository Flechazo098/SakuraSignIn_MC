...
import com.flechazo.sakura.network.RewardOptionSyncData;
...
public class RewardOptionDataManager {
    ...
    public static Path getServerConfigDirectory() {
        return new File(SakuraSignInFabric.getServerInstance().getRunDirectory(), "serverconfig" + File.separator + SakuraSignInFabric.MOD_ID).toPath();
    }
    ...
    public static RewardOptionSyncPacket toSyncPacket(PlayerEntity player) {
        ...
        for (ERewardRule rule : ERewardRule.values()) {
            ...
            if (!player.hasPermissionLevel(SakuraUtils.getRewardPermissionLevel(rule))) {
                uniqueRewards.add(new RewardOptionSyncData(rule, "", new Reward(0, ERewardType.SIGN_IN_CARD).setDisabled(true)));
            } else {
                RewardOptionDataManager.getRewardMap(rule).forEach((key, value) -> {
                    List<RewardOptionSyncData> list = value.stream()
                            ...
                            .toList();
                    uniqueRewards.addAll(list);
                });
            }
        }
        
        // 将去重后的奖励添加到数据包
        packet.getData().addAll(uniqueRewards);
    ...
    public static RewardOptionData fromSyncPacketList(List<RewardOptionSyncPacket> packetList) {
        ...
        if (FabricLoader.getInstance().getEnvironmentType() == EnvType.SERVER && 
            (packetList == null || packetList.isEmpty() ||
            packetList.stream().allMatch(packet -> packet.getData() == null || packet.getData().isEmpty()))) {
            LOGGER.debug("Empty data received on server, keeping existing reward option data");
            return rewardOptionData;
        }

        RewardOptionData result = new RewardOptionData();

        // 使用Map来存储每个规则类型的奖励
        Map<ERewardRule, Map<String, RewardList>> ruleRewardMap = new HashMap<>();

        // 合并所有数据包中的数据
        for (RewardOptionSyncPacket packet : packetList) {
            for (RewardOptionSyncData data : packet.getData()) {
                // 获取或创建该规则类型的Map
                Map<String, RewardList> rewardMap = ruleRewardMap.computeIfAbsent(data.rule(), k -> new HashMap<>());
                // 获取或创建对应key的奖励列表
                RewardList rewardList = rewardMap.computeIfAbsent(data.key(), k -> new RewardList());
                rewardList.add(data.reward());
            }
        }

        // 将合并后的数据设置到结果中
        for (Map.Entry<ERewardRule, Map<String, RewardList>> entry : ruleRewardMap.entrySet()) {
            setRewardMap(result, entry.getKey(), entry.getValue());
        }

...
public class Component implements Cloneable, Serializable {
    ...
    public @NonNull String getLanguageCode() {
        return this.languageCode == null ? SakuraSignInFabric.DEFAULT_LANGUAGE : this.languageCode;
    }
    ...
    public Style getStyle() {
        ...
        style = style.withUnderline(this.isUnderlined())
    ...
    public String getString(String languageCode, boolean igStyle, boolean igColor) {
        ...
        if (!this.isColorEmpty()) {
            ...
            if (this.i18nType == EI18nType.PLAIN) {
                ...
                result.append(((Text) this.original).getString());
            } else {
     ...
     * @param languageCode 语言代码
     */
    public Text toTextComponent(String languageCode) {
        List<MutableText> components = new ArrayList<>();
        if (this.i18nType == EI18nType.ORIGINAL) {
            components.add((MutableText) this.original);
        } else {
            ...
            if (!this.isColorEmpty()) {
                if (this.i18nType != EI18nType.PLAIN) {
                    ...
                    for (String s : split) {
                        components.add(Text.literal(s).fillStyle(this.getStyle()));
                    ...
                    components.add(Text.literal(this.text).fillStyle(this.getStyle()));
                }
            }
        }
        components.addAll(this.getChildren().stream().map(component -> (MutableText) component.toTextComponent(languageCode)).toList());
        if (components.isEmpty()){
            components.add(Text.literal(""));
        }
        MutableText result = components.get(0);
        for (int j = 1; j < components.size(); j++) {
            result.append(components.get(j));
        }
        return result.fillStyle(this.getStyle());
    }

    /**
     * 获取翻译文本组件
     */
    public Text toTranslatedTextComponent() {
            ...
            if (this.i18nType != EI18nType.PLAIN) {
                Object[] objects = this.getArgs().stream().map(component -> {
                    if (component.i18nType == EI18nType.PLAIN) {
                        return component.toChatComponent();
                    } else {
                        return component.toTranslatedTextComponent();
                    }
                }).toArray();
                if (CollectionUtils.isNotNullOrEmpty(objects)) {
                    result = Text.translatable(I18nUtils.getKey(this.i18nType, this.text), objects);
                } else {
                    result = Text.translatable(I18nUtils.getKey(this.i18nType, this.text));
                }
            } else {
                result = Text.literal(this.text).fillStyle(this.getStyle());
            }
        }
        for (Component child : this.getChildren()) {
     ...
     * @return 格式化颜色后的文本组件
     */
    public Text toChatComponent(String languageCode) {
        return rewriteColor(this.toTextComponent(languageCode));
    }

    // 😵‍💫
    public static Text rewriteColor(Text component) {
        if (component instanceof MutableText) {
            TextColor color = component.getStyle().getColor();
            if (color != null && color.getName().startsWith("#")) {
                Style style = component.getStyle().withColor(TextColor.parse(StringUtils.argbToMinecraftColor(StringUtils.argbToHex(color.getName())).name().toLowerCase()));
                ((MutableText) component).setStyle(style);
            }
        }
    ...
    public static Component deserialize(JsonObject jsonObject) {
        ...
        // 检查必需字段
        if (!jsonObject.has("text") || !jsonObject.has("i18nType") || !jsonObject.has("languageCode")) {
            throw new IllegalArgumentException("Missing required fields in Component JSON");
        }
        ...
        // 设置可选字段,使用默认值
        result.setColor(jsonObject.has("color") ? jsonObject.get("color").getAsInt() : 0xFFFFFF);
        result.setBgColor(jsonObject.has("bgColor") ? jsonObject.get("bgColor").getAsInt() : 0);
        result.setShadow(jsonObject.has("shadow") && jsonObject.get("shadow").getAsBoolean());
        result.setBold(jsonObject.has("bold") && jsonObject.get("bold").getAsBoolean());
        result.setItalic(jsonObject.has("italic") && jsonObject.get("italic").getAsBoolean());
        result.setUnderlined(jsonObject.has("underlined") && jsonObject.get("underlined").getAsBoolean());
        result.setStrikethrough(jsonObject.has("strikethrough") && jsonObject.get("strikethrough").getAsBoolean());
        result.setObfuscated(jsonObject.has("obfuscated") && jsonObject.get("obfuscated").getAsBoolean());
        ...
        if (jsonObject.has("hoverEvent")) {
            result.setHoverEvent(HoverEvent.fromJson(jsonObject.get("hoverEvent").getAsJsonObject()));
        }

        // 处理子组件和参数
        if (jsonObject.has("children") && !jsonObject.get("children").isJsonNull()) {
            for (JsonElement childJson : jsonObject.getAsJsonArray("children")) {
                if (!childJson.isJsonNull()) {
                    result.getChildren().add(deserialize(childJson.getAsJsonObject()));
                }
            }
        }
        ...
        if (jsonObject.has("args") && !jsonObject.get("args").isJsonNull()) {
            for (JsonElement argJson : jsonObject.getAsJsonArray("args")) {
                if (!argJson.isJsonNull()) {
                    result.getArgs().add(deserialize(argJson.getAsJsonObject()));
                }
            }
        }
    ...
    public static JsonObject serialize(Component reward) {
        ...
        if (reward.getHoverEvent() != null) {
            result.add("hoverEvent", reward.getHoverEvent().toJson());
        }

...
public class Component implements Cloneable, Serializable {
    ...
    public Text toTextComponent(String languageCode) {
        ...
        components.addAll(this.children.stream().map(component -> (MutableText) component.toTextComponent(languageCode)).toList());
        ...
        if (!this.isColorEmpty() || !this.isBgColorEmpty()) {
            if (this.i18nType != EI18nType.PLAIN) {
                ...
                if (CollectionUtils.isNotNullOrEmpty(args)) {
                    result = Text.translatable(I18nUtils.getKey(this.i18nType, this.text), args);
                } else {

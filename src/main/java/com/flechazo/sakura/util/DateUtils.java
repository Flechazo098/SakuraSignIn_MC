...
public class DateUtils {
    ...
    private static Date getClientValidDate (Date date) {
        Date originalTime = DateUtils.format(SakuraSignInFabric.getClientServerTime().getKey());
        Date actualTime = DateUtils.format(SakuraSignInFabric.getClientServerTime().getValue());
        if (originalTime.compareTo(actualTime) != 0){
            date = DateUtils.addDate(date, DateUtils.dateOfTwo(originalTime, actualTime));
        }
        return date;
